...
export class Admin implements OnInit {
  ...
  async sauvegarderUser(): Promise<void> {
    if (!this.editUser?.id) return;
    const data: Partial<Utilisateur> = { ...this.editUser };

    if (this.nouveauMotDePasse.trim()) {
      data.motDePasse = await this.hashService.hash(this.nouveauMotDePasse.trim());
    } else {
      delete data.motDePasse; // ne pas écraser si vide
    }

    this.utilisateurService.mettreAJour(this.editUser.id, data).subscribe({
      next: (updated) => {
